(function () {
    ...
    // alerts
    app.service("$messages", function ($rootScope, $window) {
        return {
            store: function (text, type = 'success') {
                $window.localStorage.setItem('storeMessage', JSON.stringify({ type, text }));
            },
            ...
            alert: function (message, type, time) {
                if ($.hasValue(message))
                    $.smkAlert({ text: message.nl2Br(), type: type, time: time || 5, position: 'bottom-center' });
            },
            confirm: function (message, ok, notOk, accept = "Sim", cancel = "Não") {
                if (!$.hasValue(message))
                    return;
                ...
                $.smkConfirm({
                    ...
                    if (e) {
                        $rootScope.$applyAsync(ok);
                    } else if (notOk) {
                        $rootScope.$applyAsync(notOk);
                    }
                });
            }
        }
    });
    app.directive("showMessage", function ($messages, $window) {
        return function (scope, element, attrs) {
            const storeMessage = JSON.parse($window.localStorage.getItem('storeMessage'));
            if (!storeMessage) return;

            const type = storeMessage.type;
            const time = "time" in attrs ? attrs.time.toInt() : null;

            angular.element(document).ready(function () {
                $messages[type](storeMessage.text, time);
            });

            $window.localStorage.removeItem('storeMessage')
        }
    });
    ...
    app.service("$request", function ($http, $messages, $login) {
        ...
        function parseData(data) {
            ...
            if (typeof data == "string") {
                const date = new Regex(/^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9:]{8}([.0-9]+)?$/, data);
                if (date.match)
        ...
        function generateOptions(method, args) {
            ...
            } else if (method === "GET" && typeof (options) != "object" && args.length === 3 && args[1].id) {
                options = {};
                options.url = `${args[0]}/${args[1].id}`;
                options.success = args[2];
        ...
        return {
            ...
            put: function () {
                request(generateOptions("PUT", arguments));
            },
            remove: function () {
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.onFocus = function () {
                    scope.previousName = scope.ngModel.Nome;
                };

                scope.onBlur = function () {
                    if (scope.ngModel.Nome.trim() === "")
                        scope.ngModel.Nome = scope.previousName;
            ...
            template: `<div class="my-1 form-check">
                            <input class="form-check-input" type="checkbox" ng-model="ngModel.Selecionado" ng-show="!isEditing" />
                            <div ng-show="isEditing" class="d-flex justify-content-between">
                                <input type="text" class="border-0 w-100"  ng-model="ngModel.Nome" ng-focus="onFocus()" ng-blur="onBlur()">
                                <a href="javascript:" class="me-auto" ng-click="onremove()">
                                    <i class="fa fa-times"></i>
                                </a>
                            </div>
                            <label class="form-check-label" ng-click="isEditing = true" ng-show="!isEditing">
                                {{ngModel.Nome}}
                            </label>
                        </div>`
        }
    ...
    app.directive("pagination", function () {
        return {
            restrict: "E",
            link: function (scope, element, attrs) {
                const filters = scope.$eval("{0} || ({0} = {})".format(attrs.filters));

                filters.itemsPerPage = parseInt(attrs.itemsPerPage || filters.itemsPerPage || 10);
    ...
    app.directive("dropdown", function () {
        ...
        function getParameters(attrs) {
            const items = attrs.items.match(/^((\w+)?:? in )?([\w.()]+)$/);
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                function evaluateObjectValue() {
                    ...
                    var selected = items.singleOrDefault(null, function (i) {
            ...
            template: function (element, attrs) {
                var parameters = getParameters(attrs);
                var ngOptions = getNgOptions(parameters);

                var select =
                    "<select class='form-select' ng-options='" + ngOptions + "'>\
                        <option value=''>" + parameters.defaultText + "</option>\
                    </select>";
    ...
    app.directive("enum", function ($compile, $guid) {
        ...
        function getRadioTemplate(element, attrs) {
            var guid = $guid.createElementName();
            var panelAttributes = ["ng-show", "ng-hide", "ng-if", "auto-focus-on"];

            var div = $("<div>");
            var panel = $("<div class='form-check form-check-inline'>");
            var label = $("<label ng-bind='e.Display' for='" + guid + "_{{e.Value}}'>");
            var radio = $("<input id='" + guid + "_{{e.Value}}'/>");
            ...
            radio.attr('ng-model', '$parent.' + radio.attr('ng-model'));

            if ($.hasValue(attrs.defaultText)) {
                var defaultLabel = $(label.outerHtml());
                var defaultRadio = $(radio.outerHtml());
        ...
        return {
            ...
            terminal: true,
            compile: function (element, attrs) {
                var template = "radio" in attrs
                    ? getRadioTemplate(element, attrs)
                    : getSelectTemplate(element, attrs);
    ...
    app.directive("fileUploader", function (safeApply, $messages) {
        return {
            ...
            link: function (scope, element, attrs, controls) {
                ...
                var maxSize = "maxSize" in attrs ? parseInt(attrs.maxSize) : 25;

                function getFiles(e) {
                    var files = [];
                    for (var i = 0; i < e.currentTarget.files.length; i++) {
                        if (maxSize != null) {
                            var maxSizeAllowed = maxSize * 1024 * 1024;
                            const file = e.currentTarget.files[i];

                            if (file.size > maxSizeAllowed) {
                                $messages.error(`O arquivo ${file.name} tem mais de ${maxSize}mb`);
            ...
            template: function (element, attrs) {
                ...
                const id = "fileId" in attrs ? `id="${attrs.fileId}"` : "";

                const html =
                    `<div class='input-group'>
                        <input type='text' class='form-control ${inputSize}' readonly='readonly' ng-value='fileName' style='max-width: none' required-field='isRequired()||false' />
                        <button type='button' class='btn btn-outline-success ${buttonSize}' ng-show='fileName' ng-click='clear()'>
                            <span class='fa fa-remove'></span> Limpar
                        </button>
                        <button type='button' class='btn btn-outline-success ${buttonSize}' ng-click='select()'>
                            <span class='fa fa-folder-open'></span>
                            <span class='ml-2'> {{fileName ? 'Alterar' : 'Procurar'}}</span>
                        </div>
                        <input ${id} type='file' style='display: none' accept="${attrs.accept}"/>
                    </div>`;
                    ...
                    "<div class='wizard'>\
                        ...
                        <ul class='list-inline pull-right tab-buttons'>\
                            <li ng-show='canGoBack()'>\
                                <button type='button' class='btn btn-default prev-step' ng-click='goBack()' ng-bind=\"currentStep.backText || 'Voltar'\"></button>\
                            </li>\
        ...
        return {
            ...
            link: function (scope, element) {
                ...
                scope.$watch("minDate", function (v) {
                    scope.options.minDate = v;
                });
                scope.$watch("maxDate", function (v) {
                    scope.options.maxDate = v;
                });
            },
            template: function (element, attrs) {
    ...
    app.directive("datatable", function () {
        ...
        var buttonTypes = ["info", "success", "danger", "warning", "primary", "default"];

        function getSorterPredicateFromBind(bind, itemName) {
            var regex = new RegExp(itemName + "\\.([.\\w]+)(?:$|(?:\\.\\w+\\()|\\)|\\]|\\s)", "m");
            var match = bind.match(regex);
        ...
        function createTableElement(element, attrs) {
            ...
            var repeater = table.find("tbody tr");
        ...
        function createButtonElement(button) {
            var isLink = "href" in button.attributes ||
        ...
        function getButtonType(button) {
            ...
            return buttonTypes.firstOrDefault("default", function (type) {
        ...
        return {
            ...
            compile: function (element, attrs) {
                ...
                element.find("column").each(function () {
                    var column = $(this).information();

                    var th = $("<th>");
                    var td = $("<td>");
                    td.addClass("align-middle");

                    var sorterPredicate = getSorterPredicate(column, itemName);
                ...
                if (element.find("buttons").length) {
                    ...
                    element.find("buttons > button").each(function () {
                        var button = $(this).information();
                        var btn = createButtonElement(button);
                        var icon = getButtonIconAttribute(button);
                        var destination = getButtonDestination(button);
                        ...
                        table.find("{0} tr {1}:last".format(destination.container, destination.cell)).append(" " + btn.outerHtml());
                    });
                ...
                return function (scope, element, attrs) {
                    scope.order = function (predicate) {
                        var filters = scope.$eval(attrs.filters);
