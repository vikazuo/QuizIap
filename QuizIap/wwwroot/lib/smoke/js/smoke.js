...
(function ($) {
	...
	$.fn.smkValidate = function () {
		...
		this.find('.mt-3.has-feedback.has-error').first().find(':input:not(:button):not(:disabled):not(.novalidate)').first().focus();
	...
	*/
	...
	$.smkAlert = function (options) {
		...
		switch (settings.type) {
			case 'warning':
				...
				if (settings.icon === '') settings.icon = 'fa-exclamation-circle';
				break;
			case 'success':
				settings.type = 'alert-success';
				if (settings.icon === '') settings.icon = 'fa-check-circle';
				break;
			case 'danger':
				settings.type = 'alert-danger';
				if (settings.icon === '') settings.icon = 'fa-times-circle';
				break;
			case 'info':
				settings.type = 'alert-info';
				if (settings.icon === '') settings.icon = 'fa-info-circle';
				break;
		}
		...
		var obj = $('<div class="alert alert-dismissible ' + settings.type + ' smk-alert"><button type="button" class="btn-close" data-bs-dismiss="alert"></button><span class="fa ' + settings.icon + '"></span><p>' + settings.text + '</p></div>');
	...
	$.smkConfirm = function (options, callback) {
		...
		$('body').append('<div class="smk-confirm-back"><div class="card alert-light smk-confirm" tabindex="1"><div class="card-body">' + settings.text + '</div><div class="card-footer text-end"><a class="btn btn-outline-dark btn-sm smk-cancel" href="#" >' + settings.cancel + '</a> <a class="btn btn-primary btn-sm smk-accept" href="#">' + settings.accept + '</a></div></div></div>');
